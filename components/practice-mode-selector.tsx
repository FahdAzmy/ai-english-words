...
interface PracticeModeSelectorProps {
  dayId: string;
  words: Word[];
  suggestedMode: 'story' | 'sentences' | 'dialogue' | 'writing';
}
...
const practiceModes = [
  {
    ...
    description: 'Read an engaging story that naturally incorporates all your vocabulary words. Great for understanding context and usage.',
    icon: '📚',
  ...
  },
  {
    ...
    description: 'Build meaningful sentences with your vocabulary words. Practice combining words in grammatically correct ways.',
    icon: '✍️',
  ...
  },
  {
    ...
    description: 'Engage in a simulated conversation where you use your vocabulary words naturally. Learn through dialogue.',
    icon: '💬',
    ...
    <div>
      <div className="mb-10">
        ...
        <p className="text-base text-muted-foreground max-w-2xl">
          Select a learning style that works best for you. We recommend starting with <span className="inline-block bg-primary/20 px-2 py-1 rounded text-primary font-semibold">{suggestedMode}</span> based on your progress.
        </p>
      </div>
            ...
            <Link key={mode.id} href={`/day/${dayId}/practice/${mode.id}`}>
              ...
              >
                ...
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity`}>
                  <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${mode.color} rounded-full blur-3xl`}></div>
                </div>

                <div className="relative z-10 flex items-start gap-5 sm:gap-6">
                  {/* Icon */}
                  <div className="text-5xl sm:text-6xl flex-shrink-0 group-hover:scale-110 transition-transform">{mode.icon}</div>
      ...
      <div className="mt-10 rounded-2xl bg-gradient-to-r from-accent/10 to-primary/10 border border-accent/20 p-6 backdrop-blur-sm">
        <p className="text-sm text-foreground font-bold mb-2 flex items-center gap-2">
          <span className="text-lg">💡</span> Pro Tip
        </p>
        <p className="text-sm text-muted-foreground leading-relaxed">
          Each practice mode strengthens vocabulary in different ways. Try them all to find your preferred learning style, and mix it up to reinforce concepts from multiple angles.
        </p>
      </div>
    </div>
  );
